...
var colors = [];
var coverage = $('#coverageHidden').val();
...
//   }, 100);
// }
    if (jQuery('#columns').find('.breadcrumb').length > 0) {
        jQuery('#columns').prev().find('.breadcrumb').hide();
    } else {
        jQuery('#columns').prev().find('.breadcrumb').show();
    }

    var summaries = $('.scroll-fixed');
    summaries.each(function(i) {
        var summary = $(summaries[i]);
        var next = summaries[i + 1];
        $('body').addClass("scrolltofixed");
        summary.scrollToFixed({
            // marginTop: $('#nav-container').outerHeight(true) + 10,
            limit: function() {
                var limit = 0;
                if (next) {
    ...
    // console.log($('#thumbs ul li').length);
    // if($('#thumbs ul li').length > 4 ) {
    // 	$('#thumbs').css({"overflow-y": "auto", "max-height": "455px"});
    // }
    // if ($(window).width() > 767) {
    // 	var fixmeTop = $('#image-block').offset().top;       
    // 	var $el = $('#center_column');
    // 	var footer =  $el.position().top + $el.outerHeight(true);
    ...
    var dialog = $("#dialog-form").dialog({
        ...
        { 
            $('#product .ui-widget-overlay').bind('click', function()
            { 
                $("#product #dialog-form").dialog('close'); 
            }); 
        }
    });
    ...
    if (typeof(jqZoomEnabled) != 'undefined' && jqZoomEnabled) {
        $('.jqzoom').jqzoom({
            ...
            zoomWidth: 360, //zooming div default width(default width value is 200)
            zoomHeight: 360, //zooming div default width(default height value is 200)
    ...
    //catch the click on the "reviews" button at the top of the page
    $(document).on('click', '.comments_note .reviews', function(e) {
        $('#product-tabs a[href="' + $(this).attr('href') + '"]').click();
        $.scrollTo('#product-tabs', 600);
    });
    ...
    if (contentOnly == false) {
        if (!!$.prototype.fancybox) $('li:visible .fancybox, .fancybox.shown').fancybox({
    ...
    $(document).on('click', '.product_quantity_up', function(e) {
        ...
        if (quantityAvailable > 0) quantityAvailableT = quantityAvailable;
        else quantityAvailableT = 100000000;
        if (!isNaN(currentVal) && currentVal < quantityAvailableT) $('input[name=' + fieldName + ']').val(currentVal + 1).trigger('keyup');
    ...
    $(document).on('click', '.product_quantity_down', function(e) {
        ...
        fieldName = $(this).data('field-qty');
        var currentVal = parseInt($('input[name=' + fieldName + ']').val());
        if (!isNaN(currentVal) && currentVal > 1) $('input[name=' + fieldName + ']').val(currentVal - 1).trigger('keyup');
...
function updateDisplay() {
    ...
    if (!selectedCombination['unavailable'] && quantityAvailable > 0 && productAvailableForOrder == 1) {
        ...
        if (!selectedCombination['unavailable']) {
            ...
            if (!allowBuyWhenOutOfStock) $('#availability_value').addClass('warning_inline');
        } else {
...
function refreshProductImages(id_product_attribute) {
    ...
    if (id_product_attribute > 0 && typeof(combinationImages) != 'undefined' && typeof(combinationImages[id_product_attribute]) != 'undefined') {
        ...
        for (var i = 0; i < combinationImages[id_product_attribute].length; i++)
            if (typeof(jqZoomEnabled) != 'undefined' && jqZoomEnabled) $('#thumbnail_' + parseInt(combinationImages[id_product_attribute][i])).show().children('a.shown').trigger('click');
...
function getProductAttribute() {
    ...
    for (var i in attributesCombinations)
        for (var a in tab_attributes)
            if (attributesCombinations[i]['id_attribute'] === tab_attributes[a]) request += '/' + attributesCombinations[i]['group'] + attribute_anchor_separator + attributesCombinations[i]['attribute'];
...
$(window).load(function() {
    $("#theColorname").text($('.color_pick.selected').attr('name'));
});
// Calculate Square Footage //
$("#ft1,#ft2,#in1,#in2,#wasteperc").keyup(function() {
    if ($('#ft1').val().match(/[^\d.]/g, '') || ($('#ft2').val().match(/[^\d.]/g, '')) || ($('#in1').val().match(/[^\d.]/g, '')) || ($('#in2').val().match(/[^\d.]/g, ''))) {
        if ($('#validate-calc').html().length == 0) {
            $('#validate-calc').text('Please enter digits!!');
            $('#validate-calc').show();
        }
    } else {
        $('#validate-calc').text('');
    ...
    var feet1 = parseInt($("#ft1").val() * 12);
    var feet2 = parseInt($("#ft2").val() * 12);
    var inch1 = ($("#in1").val() == '') ? 0 : $("#in1").val();
    var inch2 = ($("#in2").val() == '') ? 0 : $("#in2").val();
    var measure1 = feet1 + parseInt(inch1);
    var measure2 = feet2 + parseInt(inch2);
    var total = round(measure1 * measure2 / 144, 2);
    var unitsrequired = Math.ceil(total / coverage);
    var tenPerc = total / 10;
    var waste = round(tenPerc , 2);
    var requiredUnitAfterWaste = Math.ceil(((total + waste) / coverage), 2);
    var SqIncluded = round(requiredUnitAfterWaste * coverage, 2);
    if (isNaN(waste)) {
        var waste = 0;
    }
    if (isNaN(total)) {
    ...
    $('#total').html(total + ' sq. ft');
    $('#unitsReq').html(requiredUnitAfterWaste);
    $('#unitsReqHidden').val(Math.ceil((total + (total / 10) )/ coverage));
    $('#SqIncluded').html(SqIncluded + ' sq. ft');
    $('#SqIncludedhidden').val(SqIncluded);
    $('#totalHidden').val(round((total + waste), 2));
    $('.user-text').html("You need <b>" + total + " sqft</b>. We recommend that you order <b>" + round((total + waste), 2) + " sqft</b> (10% extra) to cover edges and tile cutting.");
});
...
$("#square_footage").keyup(function() {
    if (coverage == '') {
        alert('Coverage Not specified for this product');
    } else {
        // init the reqd variables.
        var display_price_type = $('#our_price_display').text().toLowerCase();
        var totalFeet          = $(this).val();
        var unitsrequired      = Math.ceil(totalFeet / coverage);           
        var totalSqFt          = round(unitsrequired * coverage, 2);
        var price              = parseFloat($('#our_price_display').text().replace(/[^0-9\.]+/g, ""));
        var totalPrice         = '';
        //console.log(totalFeet, unitsrequired, totalSqFt, price);
        $('#numberOfBox').text(unitsrequired);          

        // case for box or sheet
        if ((display_price_type.indexOf('box') >= 0) || (display_price_type.indexOf('sheet')) >= 0) {
            totalPrice = round(price * unitsrequired, 2).toFixed(2);
            // $('#numberOfBox').text(totalFeet);          
            $('#totalSqFt').html(totalSqFt);
            $('#totalPrice').html("$" + totalPrice);
        } else {
            // case for sqft.          
            totalPrice = round(price * totalSqFt, 2).toFixed(2);
            // $('#numberOfBox').html(Math.ceil(unitsrequired));
            $('#totalSqFt').html(totalSqFt);
            $('#totalPrice').html("$" + totalPrice);
        }      
        //var waste = round(unitsrequired * (10 / 100), 1);
        //var requiredUnitAfterWaste = Math.ceil(unitsrequired + waste, 2);
        //var requiredUnitAfterWaste = Math.ceil(unitsrequired, 2);
    }
});

$('#updateProductSqFeet').click(function() {
    $("#dialog-form").dialog("close");
    $('#numberOfBox').html($('#unitsReqHidden').val());
    $('#totalSqFt').html($('#SqIncludedhidden').val());
    $('#square_footage').val($('#totalHidden').val());
    var totalFeet = $('#SqIncludedhidden').val();
    var price = parseFloat($('#our_price_display').text().replace(/[^0-9\.]+/g, ""));
    var totalPrice = round(price * totalFeet, 2);
    $('#totalPrice').html("$" + totalPrice);
});
